      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['visitOrgName']"
                  :label="get4Label('来访单位全称')"
                  name='visitOrgName'
                  prop='visitOrgName'
                  placeholder="请选择来访单位全称"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['leadersAndPositions']"
                  :label="get4Label('来访主要领导及职务')"
                  name='leadersAndPositions'
                  prop='leadersAndPositions'
                  placeholder="请选择来访主要领导及职务"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['ourLeadersAndPositions']"
                  :label="get4Label('我司参加接待领导及职务')"
                  name='ourLeadersAndPositions'
                  prop='ourLeadersAndPositions'
                  placeholder="请选择我司参加接待领导及职务"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['visitPurpose']"
                  :label="get4Label('来访目的')"
                  name='visitPurpose'
                  prop='visitPurpose'
                  placeholder="请选择来访目的"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['actualEffect']"
                  :label="get4Label('实际效果（简述）')"
                  name='actualEffect'
                  prop='actualEffect'
                  placeholder="请选择实际效果（简述）"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('ExternalReceptionList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/smartcity/externalReception/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/smartcity/externalReception/edit':'/smartcity/externalReception/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
