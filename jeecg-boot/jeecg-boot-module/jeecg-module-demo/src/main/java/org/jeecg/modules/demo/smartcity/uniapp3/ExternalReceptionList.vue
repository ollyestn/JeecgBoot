...
import {columns} from './ExternalReceptionData';
defineOptions({
  name: 'ExternalReceptionList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/smartcity/externalReception/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/smartcity/externalReception/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
