      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['trainingTopic']"
                  :label="get4Label('课题')"
                  name='trainingTopic'
                  prop='trainingTopic'
                  placeholder="请选择课题"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['mainContent']"
                  :label="get4Label('主要内容')"
                  name='mainContent'
                  prop='mainContent'
                  placeholder="请选择主要内容"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['traingType']"
                  :label="get4Label('组织形式')"
                  name='traingType'
                  prop='traingType'
                  placeholder="请选择组织形式"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['participants']"
                  :label="get4Label('参加人员')"
                  name='participants'
                  prop='participants'
                  placeholder="请选择参加人员"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('SalesTrainingList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/smartcity/salesTraining/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/smartcity/salesTraining/edit':'/smartcity/salesTraining/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
