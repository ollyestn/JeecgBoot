...
<template>
  <PageLayout navTitle="培训统计"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './SalesTrainingData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/smartcity/salesTraining/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/smartcity/salesTraining/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
