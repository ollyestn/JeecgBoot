...
<template>
  <PageLayout navTitle="投标统计"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TenderStatisticsData';
defineOptions({
  name: 'TenderStatisticsList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/smartcity/tenderStatistics/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/smartcity/tenderStatistics/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
