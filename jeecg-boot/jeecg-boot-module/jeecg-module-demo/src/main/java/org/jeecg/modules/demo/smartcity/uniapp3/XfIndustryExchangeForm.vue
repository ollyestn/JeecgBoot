      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['otherCompany']"
...
const dataId = ref('')
const backRouteName = ref('XfIndustryExchangeList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/smartcity/xfIndustryExchange/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
   ...
   for (const field of onlyField) {
      if (values[field]) {
          ...
          const res: any = await duplicateCheck({
              tableName: 'xf_industry_exchange',
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/smartcity/xfIndustryExchange/edit':'/smartcity/xfIndustryExchange/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
