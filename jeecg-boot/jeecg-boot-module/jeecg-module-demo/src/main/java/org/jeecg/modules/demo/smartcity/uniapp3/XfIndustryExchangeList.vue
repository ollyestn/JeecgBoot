...
layout: 'default',
style: {
navigationBarTitleText: '同业交流统计',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="同业交流统计"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './XfIndustryExchangeData';
defineOptions({
  name: 'XfIndustryExchangeList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/smartcity/xfIndustryExchange/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/smartcity/xfIndustryExchange/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
