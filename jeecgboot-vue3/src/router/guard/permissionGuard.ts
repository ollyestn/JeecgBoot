...
export function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if(isOAuth2AppEnv() && to.path.indexOf("/tenantId/") != -1){
      //update-begin---author:wangshuai---date:2024-11-08---for:【TV360X-2958】钉钉登录后打开了某云，换其他账号登录后，再打开某云显示的是原来账号的应用---
      if (isOAuth2DingAppEnv()) {
        next(OAUTH2_LOGIN_PAGE_PATH);
      } else {
        next(userStore.getUserInfo.homePath || PageEnum.BASE_HOME);
      }
      //update-end---author:wangshuai---date:2024-11-08---for:【TV360X-2958】钉钉登录后打开了某云，换其他账号登录后，再打开某云显示的是原来账号的应用---
      return;
    }
    //==============================【首次登录并且是企业微信或者钉钉的情况下才会调用】==================
